...
module.exports = function (config) {
    ...
    return {
        renderCached: function(view, usecache) {
            return this.render(view, undefined, undefined, undefined, undefined, 0, usecache);
        },

        render: function(view, model, body, sections, page, depth, usecache) {
            var that = this;
            ...
            page = page || function(fileName) {
                console.log(fileName);
                return that.render(fileName, undefined, undefined, undefined, undefined, depth + 1);
            };

            if ('string' === typeof view) {
                return this.renderFromFile(view, model, body, sections, page, depth, usecache)
            }

            throw new Error("I've not implemented another path for views.  Please specify a file name for the view as a string.");
        },

        renderFromFile: function(fileName, model, body, sections, page, depth, usecache) {
            if (!fileName) {
            ...
            if (usecache) {
                ...
                if (!r) {
                    r = this.renderFromString(fs.readFileSync(fileName).toString(), model, body, sections, page, depth);
                    cachedFiles[fileName] = r;
                }
                return r;
            }

            return this.renderFromString(fs.readFileSync(fileName).toString(), model, body, sections, page, depth);
        },

        renderFromString: function(contents, model, body, sections, page, depth) {
            var funcResult = this.parse(contents);
            var result = funcResult.razor.render(model, body, sections, page);
        ...
        parse: function(contents) {
            ...
            var result = '(function() { return { render: function(model, renderBody, renderSection, renderPage) { var r = "";';
            var sections = { };
